...
api.interceptors.response.use((response) => response, async (error) => {
  if (error.response?.status === 401) {
    ...
    showToast('Session expired. Please log in again.', 'warning', 3000);
...
export const dashboardService = {
  ...
  exportHistory: (period = 30): Promise<AxiosResponse<Blob>> =>
    api.get('dashboard/export', { params: { period }, responseType: 'blob' }),
};
...
export const iotService = {
  ...
  getZones: (): Promise<AxiosResponse<Zone[]>> => api.get('zones/'),
  ...
  deleteZone: (id: string): Promise<AxiosResponse<unknown>> => api.delete(`zones/${id}/`),
